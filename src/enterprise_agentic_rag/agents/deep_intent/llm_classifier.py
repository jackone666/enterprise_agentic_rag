...
你需要判断并输出以下 JSON 结构：
{
  "primary_intent": "concept_qa | api_usage | code_generation | error_diagnosis | migration | compatibility | project_debug | best_practice | architecture | learning_guidance",
  "secondary_intents": [],
  "scenario": "",
  "user_goal": "",
  "query_focus": "",
  ...
  "suggested_tools": [],
  "retrieval_plan": {
    "mode": "hybrid_only | parallel | graph_first | error_first | code_first",
    "sources": [],
    "filters": {},
    "expanded_query": null
  },
...
2. 用户贴代码或描述项目现象（白屏、黑屏、启动失败、首页打不开等），优先 project_debug。
3. 用户要求写代码、生成示例、封装，必须包含 code_generation。
4. 用户提到迁移、升级、替换、废弃 API，优先 migration。
5. 用户提到 API Level、HarmonyOS NEXT、OpenHarmony 版本问题，加入 compatibility。
6. 不要轻易追问。能先给通用答案时 needs_clarification=false，但 missing_context 要列出缺失信息。
7. 如果没有关键上下文无法定位问题（例如只知道白屏但不知道任何版本/环境），needs_clarification=true。
8. suggested_tools 必须从允许工具列表中选择。
9. retrieval_plan.mode 必须从允许模式中选择。
10. 如果检测到错误码或报错信息，scenario 应准确反映错误类型。
11. 如果有迁移关系，entities 中 migration_from 和 migration_to 必须填充。
...
允许 retrieval_plan.mode：hybrid_only, parallel, graph_first, error_first, code_first
