...
```text
              ...
              ▼                  ▼                  ▼
    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
     │  ToolAgent  │    │ Knowledge   │    │  RetrievalAgent │
     │  工具调用    │    │  知识问答    │    │  检索编排      │
     │  工单/状态   │    │  检索+生成   │    │  模式路由      │
     │  安全策略    │    │  证据融合    │    │  缓存命中 │
    └──────┬──────┘    └──────┬──────┘    └──────┬──────┘
...
| **黑板模式（Blackboard）** | ✅ | ~30 字段的 AgentState 天然是共享上下文，Agent 间零 RPC 通信 |
...
| **CodeExecutor** | `code_execution_tool.py` | 代码沙箱执行 | 六层安全沙箱、执行验证 | `code_snippet`, `code_verified`, `code_language`, `code_execution_result` |
...
所有 Agent 通过 `AgentState`（~30字段 TypedDict）共享上下文，不直接 RPC 调用：
...
**职责：** 将用户自然语言问题分类为 **6 种** 预定义意图（`schema.py:18-29` `IntentCategory` 枚举），并输出 **3 种检索模式** 之一（`RetrievalMode` 枚举）。

**6 种意图（`IntentCategory`）：**
- `concept_qa` — 概念定义/原理解释
- `api_usage` — API 用法/SDK 参数
- `code_generation` — 代码生成/示例
- `error_diagnosis` — 错误码/异常诊断（含 project_debug 合并）
- `migration` — 迁移/废弃 API 替换
- `compatibility` — 版本/API Level 兼容

**3 种检索模式（`RetrievalMode`）：**
`hybrid_only / parallel / graph_first`
...
#### 4.2.1 6 意图 → 3 检索模式 → 6 Agent 路径 完整决策矩阵

> **易误解点**：6 IntentCategory / 3 RetrievalMode / 6 Agent 路径 是**三个正交维度**，不是 1-to-1-to-1 对应。

**6 意图 → 3 模式**（`rules.py:387-406` `_suggest_mode`）：
...
| `error_diagnosis` | **`graph_first`** | 错误码/异常优先，图谱关系辅助定位 |
| `migration` | **`graph_first`** | 迁移链路强依赖实体关系（图谱） |
| `compatibility` | **`graph_first`** | 版本/API Level 兼容靠图谱 1-2 hop |

> **6 意图 → 3 模式 = 多对一**（6 个里只有 3 个不同值）。

**3 模式 → 1 BaseRAGWorkflow**（`graph/nodes/retrieval.py:135-142` 全部走 RAG；v3.1 拆分后位置）：

| RetrievalMode | Workflow | 召回策略 |
|---|---|---|
| `hybrid_only` | `BaseRAGWorkflow(mode="hybrid_only")` | Milvus + ES 并行，权重 0.5/0.5 |
| `parallel` | `BaseRAGWorkflow(mode="parallel")` | 同上，但可调权重 |
| `graph_first` | `BaseRAGWorkflow(mode="graph_first")` | 向量 + ES + **Neo4j 1-2 hop 优先** |

> **3 mode 都进入 `retrieve_knowledge` 节点，全部走 RAG**。区别仅是召回策略权重 + 是否走图谱。
...
graph TB
    ...
    IM -->|concept_qa / compatibility| HM[hybrid_only]
    IM -->|api_usage / code_generation| PM[parallel]
    IM -->|migration / error_diagnosis| GM[graph_first]
    HM --> HRW[BaseRAGWorkflow]
    PM --> HRW
    GM --> GFW[BaseRAGWorkflow]
    DI --> MK{Master._requires_tools?}
    MK -->|True| TA[ToolAgent 先走]
    TA --> HRW & GFW & EFW
    HRW & GFW & EFW --> BC[build_context]
    BC -->|code_generation| GC[CodeGenerator + execute_code]
...
3. **3 个 RetrievalMode 都走 RAG 是设计意图**：RAG 兜底保障 + Agent 增强体验。**不是"6 个 Agent 模式"**——6 Agent 和 3 RetrievalMode 是解耦的两套分类
...
- **Agent**：LLM + 工具调用 + 记忆系统 + 规划能力的综合体。本项目通过 LangGraph 执行状态机，通过 `MasterAgent` 调度 `ToolAgent`、`RetrievalAgent`、`KnowledgeAgent`、`CodeExecutor`、`VerifierAgent`；检索能力作为 `Knowledge Retrieval Service`（3层回退链+BaseRAGWorkflow）提供证据，不单独定义为 Agent。所有 worker 共享全局 AgentState。
...
**面试说明：** 先讲 CodeExecutor 不只是生成代码，还要做符号提取、沙箱执行和结果校验；Harness 负责把交付链路工程化。
...
项目采用**主从 Agent 架构**，而不是自由多 Agent 互聊。原因是企业知识问答更看重稳定性、可控性、可审计性。`MasterAgent` 统一调度（LLM 优先 + 规则兜底），运行时 worker 包括 `tool_agent`、`retrieval_service`、`answer_agent`、`verifier_agent`；其中 `CodeGenerator`（prompt utility）和 `CodeExecutor` 是代码相关能力模块，分别支撑 `generate_code` 和 `execute_code` 节点。

**P1-P3 实施后的关键变化：**

- `last_slave_agent` → `last_worker` 全部重命名，消除历史兼容字段
- CodeAgent 能力已拆分为 CodeGenerator（prompt utility）+ CodeExecutor（agent）
- MasterAgent 已从纯规则决策升级为 **LLM 优先 + 规则兜底**
- 检索从单一路径升级为**意图感知 BaseRAGWorkflow（mode 参数）分派**
- VerifierAgent 升级为 **Claim-level 断言级校验**

这样设计解决了职责重叠和不可控循环问题：意图识别只由 Deep Intent 做，路由只由 MasterAgent 做，RAG 只做证据，代码生成由 CodeGenerator 支撑，代码执行由 CodeExecutor，答案校验由 VerifierAgent 做。
...
| CodeGenerator | 能力模块 | 代码生成 prompt 工具，AST 符号提取 | 不负责执行 | 文件为 `agents/code_agent.py` |
...
- ✅ **已解决** — CodeAgent 已拆分为 CodeGenerator（prompt utility）+ CodeExecutor（agent）
...
| CodeExecutor 独立化 | 提升代码任务维护性 | code_task_success_rate 提升 10% | ✅ 已拆分 CodeGenerator + CodeExecutor |
| MasterAgent 决策评估集 | 降低路由错误 | master_route_accuracy ≥ 0.92 | ✅ 已实施 (`evals/agent_decision_eval.py`, 8条用例) |
