...
| 离线回归评估 | 22 条 Agent 决策用例 | `evals/agent_decision_eval.py` |
...
22 条 Agent 决策用例 + 8 条 Eval Gate 用例 **足够**：
- 多了 CI 跑得慢
- 多了容易引入相关性（一条挂了带挂一片）
...
- **Agent 决策评估集** (`evals/agent_decision_eval.py`)：22 条用例覆盖全部 10 种意图，评估 MasterAgent 路由准确性（intent/routing/mode 三个维度）。
...
```text
  ...
  人工评审 / 自动归因
         ...
         ▼
  补充到 Eval Dataset:
    • data/eval/regression_cases.jsonl  (11 条回归集)
    • evals/agent_decision_eval.py     (22 条决策集)
         │
         ▼
  本地修复 → 提交 PR
         │
...
| 指标 | 含义 | 建议目标 | 评估集 |
|------|------|----------|--------|
| intent_macro_f1 | 意图分类准确率 | ≥ 0.90 | Agent决策评估集(22条) |
| route_success_rate | MasterAgent路由准确率 | ≥ 0.92 | Agent决策评估集(22条) |
...
- 评测集规模和覆盖面仍偏小，难覆盖所有业务边界。（**部分改善**: Agent 决策评估集新增 22 条用例，Eval Gate 新增 8 条用例）
...
#### 2. Eval Gate 8 条 + Agent Decision 22 条 CI 流程

```mermaid
graph TB
    A[PR 提交] --> B[CI 触发]
    B --> C[跑 8 条 Eval Gate]
    B --> D[跑 22 条 Agent Decision]
    C --> E{全部通过?}
    D --> F{全部通过?}
    E -->|是| G[合并 PR]
    E -->|否| H[阻断 + 报错]
