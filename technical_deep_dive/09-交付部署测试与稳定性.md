...
| `test_master_agent.py` | MasterAgent 路由 | LLM-first 路由 + 规则兜底、6 个下游节点分发、决策评估 |
| `test_deep_intent.py` | Deep Intent | 6 类意图分类、规则+LLM、Validator 兜底 |
| `test_llm_classifier.py` | LLM 分类器 | DeepIntent LLM 路径、JSON schema 校验 |
| `test_knowledge_agent.py` | KnowledgeAgent | 模板生成、LLM 生成、引用注入 |
| `test_code_agent.py` | CodeExecutor | 多语言代码生成、AST 符号注入 |
...
| `test_graph_router.py` | 动态检索路由 | 3 种路由模式（parallel/hybrid_only/graph_first）|
...
Code Agent（CodeExecutor）是面向开发者场景新增的 Agent，与 Knowledge Agent 协同工作：

**工作流路径**：
```
build_context
  ├─ code_required=True → CodeGenerator → CodeExecutor
  │    ├─ success → generate_answer (含已验证代码)
  │    ├─ failed + retry(max 2) → CodeGenerator (修复循环)
  │    └─ failed + exhausted → generate_answer (含免责声明)
  └─ code_required=False → generate_answer (正常路径)
```

**核心设计**：
- 代码生成：CodeGenerator（prompt utility）+ 文档代码块提取 fallback
- 沙箱执行：CodeExecutor（agent）+ subprocess + 安全黑名单 + 超时/内存限制
...
降级是系统最核心的韧性保障。本项目实现了**三层递进降级链**，每一层失败自动滑入下一层，保证系统始终有可用路径：
...
| L1 BaseRAGWorkflow | mode 参数区分模式 + Cross-Encoder | ~1-3s | ~85% | 多路召回均为空或全部低分 |
| L2 失败返回空证据 | human_fallback payload 含检索状态 | ~50ms | 100% | 永不（终极兜底） |
...
| 三级降级检索链 | 检索始终有兜底 | fallback_rate < 5% | ✅ 已实施（语义缓存→BaseRAGWorkflow→失败返回空证据） |
...
graph TB
    A[请求] --> B[语义缓存命中?]
    B -->|命中| C[直接返回]
    B -->|未命中| D[BaseRAGWorkflow]
    D -->|成功| E[生成→校验→返回]
    D -->|失败| F[返回空证据 human_fallback]

    D --> G[CodeExecutor 生成]
    D --> H[KnowledgeAgent 生成]
    G & H --> I[Verifier]
