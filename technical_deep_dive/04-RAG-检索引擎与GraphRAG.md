...
### 易误会点 1：3 种 RetrievalMode ≠ 6 种 Agent 模式

| 维度 | 数量 | 是什么 | 谁产出 |
|------|------|--------|--------|
| IntentCategory | **6** | "用户在问什么"（认知层） | Deep Intent |
| RetrievalMode | **3** | "用什么方式查"（检索策略） | `_suggest_mode()` 映射 |
| Agent 路径 | **6** | "执行什么动作"（执行层） | MasterAgent 路由 |

> 三者**正交**，不是 1-to-1-to-1。3 mode 全部走 RAG，区别只是召回权重 + 是否走图谱。详见 `02-Agent设计.md` §四.5。
...
```
Deep Intent (6种意图) → deep_intent.retrieval_plan (3种模式)
  ├── hybrid_only / parallel → BaseRAGWorkflow (kw+vec并行→RRF融合→重排→证据选择)
  ├── graph_first → BaseRAGWorkflow (图谱→扩展→kw+vec并行→三路融合)
```
...
1. **覆盖更多场景**：原有 9 条规则只覆盖 3 种 mode（parallel / graph_first / hybrid_only）。升级后覆盖 3 种 mode + 1 个 BaseRAGWorkflow（mode 参数区分），更简洁
2. **BaseRAGWorkflow 统一**：通过 mode 参数统一管理所有检索策略，互不干扰
3. **Deep Intent 驱动**：意图识别（6种意图）+ 实体提取 + 置信度共同决定检索策略，比单纯的关键词正则匹配更精准
