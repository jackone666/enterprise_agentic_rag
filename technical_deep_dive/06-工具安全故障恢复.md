...
**本项目答案（v3.0 更新，评分 9/10）：** 项目在 RAG 检索中使用广泛并行：① 4个意图感知工作流内部均并行执行多种检索（keyword+vector+graph+external 四路 asyncio.gather 并发）；② Cross-Encoder 批处理（20条/批）并行打分；③ 语义缓存检查与检索并行（缓存命中直接短路）。Tool Agent 目前仍是顺序执行，但检索阶段的并行已显著降低整体延迟。
